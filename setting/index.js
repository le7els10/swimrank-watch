...
function renderLeaderboard(globalData, discId, syncAgo) {
  ...
  const isDistTime = !['maxt', 'maxd'].includes(discId)
...
function renderRow(entry, index, discId) {
  ...
  const accentColor = isYou ? '#00BFFF' : isPodium ? medalColors[rank] : '#666'
  ...
  return View(
    ...
    [
      ...
      View(
        ...
        [
          View(
            { style: { fontSize: isPodium ? '18px' : '15px', fontWeight: 'bold', color: accentColor } },
